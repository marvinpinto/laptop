...
show_help() {
  ...
  echo "   Environment variables & defaults:"
  echo "     - VERBOSE: yes|<empty> <default: empty>"
  echo "-f <video file>: Finalize the video file (renaming, write file metadata, etc)"
...
finalize_video_file() {
  ...
  local metadata_title=$(ffprobe -v error -show_entries format_tags=title -of default=nw=1:nk=1 "${temp_video_dir}/${filename}.mp4")
  if [ -z "${metadata_title}" ]; then
    echo "File ${video_file} does not appear to have its mp4 container \"title\" tag set."
    exit 1
  fi

  local metadata_artist=$(ffprobe -v error -show_entries format_tags=artist -of default=nw=1:nk=1 "${temp_video_dir}/${filename}.mp4")
  if [ -z "${metadata_artist}" ]; then
    echo "File ${video_file} does not appear to have its mp4 container \"artist\" tag set."
    exit 1
  fi

  local metadata_date=$(ffprobe -v error -show_entries format_tags=date -of default=nw=1:nk=1 "${temp_video_dir}/${filename}.mp4")
  if [ -z "${metadata_date}" ]; then
    echo "File ${video_file} does not appear to have its mp4 container \"date\" tag set."
    exit 1
  fi

  local metadata_creation_time=$(ffprobe -v error -show_entries format_tags=creation_time -of default=nw=1:nk=1 "${temp_video_dir}/${filename}.mp4")
  if [ -z "${metadata_creation_time}" ]; then
    echo "File ${video_file} does not appear to have its mp4 container \"creation_time\" tag set."
    exit 1
  fi

  local metadata_comment=$(ffprobe -v error -show_entries format_tags=comment -of default=nw=1:nk=1 "${temp_video_dir}/${filename}.mp4")
  if [ -z "${metadata_comment}" ]; then
    echo "File ${video_file} does not appear to have its mp4 container \"comment\" tag set to a date/time value (this is used to set the EXIF date/time values)"
    exit 1
  fi

  echo "- Renaming & adding EXIF data"
  $EXIFTOOL -overwrite_original "-AllDates=${metadata_comment}" "${temp_video_dir}/${filename}.mp4"
  local filename_title=$(echo "$metadata_title" | tr '[:upper:]' '[:lower:]' | sed -e 's/[^a-z._-]/_/g')
  $EXIFTOOL  -overwrite_original '-FileName<DateTimeOriginal' -d "./%Y-%m-%d_%H.%M.%S%%-c-${filename_title}.%%le" "${temp_video_dir}/${filename}.mp4"
}
